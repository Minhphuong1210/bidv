...
$(document).ready(function () {
    ...
    if (btnSingle) {
        btnSingle.onclick = function () {
            ...
            let bank = document.getElementById('bank_single').value;
            if (!name || !bank) return alert('Vui lòng nhập tên và chọn ngân hàng');
            ...
            fetch('/va/create', {
                ...
                .then(res => {
                    if (res.status === 'success') {
                        $('#modalVa').text(res.data.va_number || '');
                        $('#modalma_don_hang').text(res.data.ma_don_hang);
                        $('#modalBank').text(res.data.bank || bank);
                        $('#modalName').text(res.data.merchant_name || name);
                        $('#modalFee').text('CK: ' + (res.data.fee_rate || 8) + '%');

                        // if (res.data.quick_link) {
                        //     $('#modalQrImage').attr('src', res.data.quick_link);
                        //     $('#modalQrContainer').show();
                        // } else {
                        //     // Fallback to generated QR if quick_link is somehow missing
                        //     $('#modalQrImage').attr('src', res.data.quick_link);
                        //     $('#modalQrContainer').show();
                        // }
                        ...
                        $('#result').html('<div style="color:red">✖ ' + res.message + '</div>');
                    }
                })
                .finally(() => {
                    btnSingle.disabled = false;
                    btnSingle.innerHTML = 'Tạo 1 tài khoản';
                });
        };
    }
            ...
            preConfirm: () => {
                return new Promise((resolve, reject) => {
                    $.post('/user/withdraw', { _token: csrf, bank_id: bank_id }, function (res) {
                        ...
                        resolve({ status: 'error', message: 'Lỗi kết nối, vui lòng thử lại!' });
                    });
                });
            },
